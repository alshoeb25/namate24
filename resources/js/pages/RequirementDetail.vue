    ...
    <div class="bg-white rounded-xl shadow-lg p-8">
        ...
        <div class="flex items-start justify-between">
          ...
          <div class="flex flex-col items-end gap-2">
            ...
            <div v-else-if="requirement.unlock_price" class="px-4 py-2 bg-yellow-100 text-yellow-600 rounded-full text-sm font-medium">
              <i class="fas fa-lock"></i> {{ userHasSubscription() ? 'Free with Subscription' : 'Subscription Required' }} to unlock
            </div>
      ...
      <div class="flex gap-3 pt-6 border-t border-gray-200">
        ...
        >
          ...
          {{ unlocking ? 'Unlocking...' : requirement.lead_info?.is_full ? 'No Spots Available' : (userHasSubscription() ? `Unlock Contact (-₹${requirement.unlock_price || 49})` : 'Subscribe to Unlock') }}
        </button>
        <button 
          v-if="requirement.has_unlocked"
          @click="contact" 
